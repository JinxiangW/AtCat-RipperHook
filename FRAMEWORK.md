...
**Symbol annotation** (`Il2CppAsmAnnotator`, run on each `PrintAssembly` result under the same lock): raw addresses are meaningless (`call 10278DB0h`), so we resolve every hex token (MASM `…h` or `0x…`) and append a trailing `; <symbol>` comment, keeping the address for IDA/Ghidra cross-ref. Resolvers, in order: ① `appContext.MethodsByAddress[addr]` → managed method (`Cloth__base::checkRequirements`, `UnityEngine.Debug::Log`); ② key functions — reflect the `ulong` members of `appContext.GetOrCreateKeyFunctionAddresses()` into an addr→name map (`il2cpp_codegen_initialize_method`, `il2cpp_runtime_class_init_export`, …); ③ `LibCpp2IlMain.GetLiteralByAddress(addr)` → string literal (the actual game text), then `GetAnyGlobalByAddress(addr)` → `MetadataUsage` (`.Type`/`.Value`) for TypeInfo / method / field globals. Guard `addr < 0x10000` to skip register-relative offsets and the 8-bit reg names (`ah`/`bh`). Unresolved addresses (per-method once-init flags, RGCTX tokens, in-method branch targets) stay raw — they aren't metadata usages so all resolvers return null, which is the desired pass-through.
